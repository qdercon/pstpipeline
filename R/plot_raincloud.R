...
plot_raincloud <- function(summary_df,
  ...
  rain_plot <- rain_plot +
    geom_flat_violin(position = ggplot2::position_nudge(x = .075, y = 0),
                     adjust = 2, trim = FALSE) +
  ...
  if (type == "parameter") {
    if (length(unique(df[[type]])) == 2) {
      ...
      if (test) {
        labs <-
          c(expression(alpha*minute[rlang::sym(alpha_par_nms[1])]),
            expression(alpha*minute[rlang::sym(alpha_par_nms[2])]),
            expression(beta*minute))
      }
      else {
        labs <-
          c(expression(alpha[rlang::sym(alpha_par_nms[1])]),
            expression(alpha[rlang::sym(alpha_par_nms[2])]),
            expression(beta))
      }
    }
    rain_plot <- rain_plot +
