...
data {
  ...
  array[N] row_vector[T] affect_prev; // previous rating (of same kind)
  array[N, T] int question;           // from 1 to 3 (happy, confident, engaged)
  array[N] row_vector[T] int_time;    // intervening time between ratings
}
...
transformed data {
  ...
  array[N] vector[T] int_time_tr;
  for (i in 1:N) {
    int_time_tr[i] = to_vector(int_time[i]);
  }
}
...
transformed parameters {
  ...
  for (q in 1:3) {
    ...
    w1_i[:, q]  = mu_wt[q, 2] + sigma_wt[q, 2] * w1_i_pr[:, q];
...
model {
  ...
  for (i in 1:N) {
    ...
    vector[ti] ev_vec;       // Vector of expected values
    vector[ti] pe_vec;       // Vector of prediction errors
