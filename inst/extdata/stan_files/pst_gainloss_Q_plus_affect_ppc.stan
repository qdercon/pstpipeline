...
model {
  ...
  for (p in 1:3) {
    ...
    w1[:, p] ~ normal(mu_wt[2, p], sigma_wt[2, p]);
    w2[:, p] ~ normal(mu_wt[3, p], sigma_wt[3, p]);
    w3[:, p] ~ normal(mu_wt[4, p], sigma_wt[4, p]);
  }
...
generated quantities {
  ...
  real<lower=0,upper=1>  mu_w0[3];
  real<lower=0,upper=1>  mu_w1[3];
  real<lower=0,upper=1>  mu_w2[3];
  real<lower=0,upper=1>  mu_w3[3];
  ...
  for (p in 1:3) {
    ...
    mu_w1[p] = Phi_approx(mu_wt[2, p]);
    mu_w2[p] = Phi_approx(mu_wt[3, p]);
    mu_w3[p] = Phi_approx(mu_wt[4, p]);
  }
