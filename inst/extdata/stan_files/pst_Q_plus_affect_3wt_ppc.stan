...
model {
  ...
  alpha ~ normal(mu_ql_pr[1], sigma_ql[1]);
  beta  ~ normal(mu_ql_pr[2] * 10, sigma_ql[2] * 10);

  // priors on the weights
  for (p in 1:3) {
    w0[:, p]   ~ normal(mu_wt[1, p], sigma_wt[1, p]);
    w2[:, p]   ~ normal(mu_wt[2, p], sigma_wt[2, p]);
    w3[:, p]   ~ normal(mu_wt[3, p], sigma_wt[3, p]);
  }
...
generated quantities {
  ...
  mu_alpha = Phi_approx(mu_ql_pr[1]);
  mu_beta  = Phi_approx(mu_ql_pr[2]) * 10;

  for (p in 1:3) {
    mu_w0[p]    = Phi_approx(mu_wt[1, p]);
    mu_w2[p]    = Phi_approx(mu_wt[2, p]);
    mu_w3[p]    = Phi_approx(mu_wt[3, p]);
