// -----------------------------------------------------------------------------
// Q-learning model for PST training data + affect
//------------------------------------------------------------------------------
// References:
...
functions {
  ...
  real partial_sum_lpmf(array[] int seq_i, int start, int end,
    ...
    for (n in 1:size(seq_i)) {
      ...
      vector[ti] mu = affect_mu(reg, qn, w0[i], w2[i], w3[i]);
...
model {
  ...
  if (!prior_only) {
    target += reduce_sum(partial_sum_lupmf, seq_i, grainsize,
                         ...
                         affect_tr, alpha, beta, w0, w2, w3, gamma, phi);
  }
}
...
generated quantities {
  ...
  vector[3] mu_w2    = mu_wt[ : , 2];
  vector[3] mu_w3    = mu_wt[ : , 3];
