...
// dual reward/loss sensitivities
//------------------------------------------------------------------------------
// References:
...
transformed parameters {
  ...
  vector<lower=0>[N] rho_pos;
  vector<lower=0>[N] rho_neg;

  alpha   = Phi_approx(mu_ql[1] + sigma_ql[1] * alpha_pr);
  rho_pos = exp(mu_ql[2] + sigma_ql[2] * rho_pos_pr);
  rho_neg = exp(mu_ql[3] + sigma_ql[3] * rho_neg_pr);
...
model {
  ...
  alpha_pr   ~ normal(0, 1);
  rho_pos_pr ~ normal(0, 1);
  rho_neg_pr ~ normal(0, 1);
  ...
  for (i in 1:N) {
    ...
    real reward_val;         // Reward value (after multiplying by sensitivity)
    ...
    for (t in 1:ti) {
      ...
      // reward multiplied by reward sensitivity when 1 and by punishment sensitivity when -1
      reward_val = (reward[i, t] == 1) ? rho_pos[i] : -rho_neg[i];
    ...
    choice[i, :ti] ~ bernoulli_logit(delta);
...
generated quantities {
  ...
  real<lower=0> mu_rho_pos;
  real<lower=0> mu_rho_neg;
  ...
  mu_rho_pos = exp(mu_ql[2]);
  mu_rho_neg = exp(mu_ql[3]);
  ...
  for (i in 1:N) {
    ...
    log_lik[i] += bernoulli_logit_lpmf(choice[i, :ti] | delta);
