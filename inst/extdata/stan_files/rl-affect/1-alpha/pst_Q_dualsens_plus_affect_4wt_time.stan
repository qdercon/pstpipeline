...
functions {
  matrix pst_regressors(int ti,
    ...
    for (t in 1:ti) {
      ...
      // reward multiplied by reward sensitivity when 1, punishment sensitivity when -1
      reward_val = (reward_i[t] == 1) ? rho_pos_i : -rho_neg_i;
      pe = reward_val - ev[co];
  ...
  real partial_sum_lpmf(array[] int seq_i, int start, int end,
    ...
    for (n in 1:size(seq_i)) {
      ...
      lp += bernoulli_logit_lupmf(choice[i, 1:ti] | reg[ : , 1]);
...
model {
  ...
  if (!prior_only) {
    target += reduce_sum(partial_sum_lupmf, seq_i, grainsize,
                         ...
                         affect_tr, ovl_time_tr, alpha, rho_pos, rho_neg,
...
generated quantities {
  ...
  real<lower=0> mu_rho_pos = exp(mu_ql[2]);
  real<lower=0> mu_rho_neg = exp(mu_ql[3]);
  ...
  if (run_gq) {
    for (i in 1:N) {
      ...
      ll_choice_i = bernoulli_logit_lpmf(choice[i, 1:ti] | reg[ : , 1]);
