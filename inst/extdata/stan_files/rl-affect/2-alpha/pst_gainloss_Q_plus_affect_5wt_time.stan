...
functions {
  ...
  vector affect_mu(matrix reg,
    ...
    return inv_logit(to_vector(w0_i[question_i])
                     ...
                     + to_vector(w1_b_i[question_i]) .* blk_time_i
  ...
  real partial_sum_lpmf(array[] int seq_i, int start, int end,
    ...
    for (n in 1:size(seq_i)) {
      ...
      matrix[ti, 3] reg = pst_regressors(ti, option1[i], option2[i], choice[i],
                                         reward[i], question[i],
                                         alpha_pos[i], alpha_neg[i], gamma[i]);

      vector[ti] mu = affect_mu(reg, qn, w0[i], w1_o[i], w1_b[i], w2[i], w3[i],
                                ovl_time_tr[i][1:ti], blk_time_tr[i][1:ti]);
...
data {
  ...
  array[N] row_vector<lower=0>[T] blk_time;        // in hours
}
...
model {
  ...
  if (!prior_only) {
    target += reduce_sum(partial_sum_lupmf, seq_i, grainsize,
                         ...
                         affect_tr, ovl_time_tr, blk_time_tr,
                         alpha_pos, alpha_neg, beta,
                         w0, w1_o, w1_b, w2, w3, gamma, phi);
  }
}
...
generated quantities {
  ...
  vector[3] mu_w1_b  = mu_wt[ : , 3];
  vector[3] mu_w2    = mu_wt[ : , 4];
  vector[3] mu_w3    = mu_wt[ : , 5];
  ...
  vector[3] w1_b_diff  = mu_w1_b[bsl]  - mu_w1_b[comp];
  ...
  if (run_gq) {
    for (i in 1:N) {
      ...
      vector[ti] mu     = affect_mu(reg, qn, w0[i], w1_o[i], w1_b[i], w2[i],
                                    w3[i], ovl_time_tr[i][1:ti],
                                    blk_time_tr[i][1:ti]);
