// -----------------------------------------------------------------------------
// Gain-loss Q-learning model for PST training data + affect + trial number
//------------------------------------------------------------------------------
// References:
...
functions {
  ...
  vector affect_mu(matrix reg,
    ...
    return inv_logit(to_vector(w0_i[question_i])
                     + to_vector(w1_o_i[question_i]) .* ovl_trial_i
...
data {
  ...
  array[N, T] int<lower=1> ovl_trial;              // from 1 to 360
}
...
transformed data {
  ...
  // transpose trial number for efficient indexing
  array[N] vector[T] ovl_trial_tr;
  for (i in 1:N) {
    ovl_trial_tr[i] = to_vector(ovl_trial[i]);
  }
}
...
model {
  ...
  if (!prior_only) {
    target += reduce_sum(partial_sum_lupmf, seq_i, grainsize,
                         ...
                         affect_tr, ovl_trial_tr, alpha_pos, alpha_neg, beta,
