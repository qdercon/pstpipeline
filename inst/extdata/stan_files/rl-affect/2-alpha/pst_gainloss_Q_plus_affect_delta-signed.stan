...
functions {
  /* Choice regressor plus per-trial (pre-update) EV and signed PE for one
  ...
  matrix pst_regressors(int ti,
                        ...
                        real alpha_pos_i,
                        real alpha_neg_i) {
    matrix[ti, 4] out; // [ ,1]=delta, [ ,2]=ev_vec, [ ,3]=pos_pe_vec, [ ,4]=neg_pe_vec
  ...
  /* Linear predictor for the affect model: baseline + overall-time slope,
     plus a lag-weighted sum of EV/signed-PE over the trials intervening
     since the previous rating of the same question (including the current
     trial). */
    ...
    for (t in 1:ti) {
      ...
      for (j in 1:int_trials_i[t]) {
        ...
        e += w2_i[question_i[t1], j] * ev_vec[t1]
           + w3_pos_i[question_i[t1], j] * pos_pe_vec[t1]
           + w3_neg_i[question_i[t1], j] * neg_pe_vec[t1];
      }
      eta[t] = e;
    }
    return eta;
...
model {
  ...
  for (q in 1:3) {
    ...
    sigma_w3_pos_pr[:, q] ~ exponential(1);
    sigma_w3_neg_pr[:, q] ~ exponential(1);
    phi_pr[:, q]          ~ normal(0, 1);
    for (j in 1:I) {
  ...
  if (!prior_only) {
    target += reduce_sum(partial_sum_lupmf, seq_i, grainsize,
                         ...
                         alpha_pos, alpha_neg, beta, w0, w1_o,
                         w2, w3_pos, w3_neg, phi);
  }
}
